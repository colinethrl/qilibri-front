...
export class Post {
    ...
    static apiToModel(postFromApi) {
        ...
        post.publishedAtString = postFromApi.published_at ? moment.utc(postFromApi.published_at).local().format('MMMM Do YYYY, h:mm a') : null;
        post.publishedAtDate = postFromApi.published_at ? new Date(postFromApi.published_at) : null;
        post.createdAt = postFromApi.created_at ? moment.utc(postFromApi.published_at).local().format('MMMM Do YYYY, h:mm a') : null;
